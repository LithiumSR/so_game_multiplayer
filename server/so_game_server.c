...
void sendDisconnect(int socket_udp, struct sockaddr_in client_addr) {
  ...
  debug_print(
      "[UDP_Receiver] Sent PostDisconnect packet of %d bytes to "
      "unrecognized user \n",
      ret);
}
...
int UDPHandler(int socket_udp, char *buf_rcv, struct sockaddr_in client_addr) {
  ...
  switch (ph->type) {
    case (VehicleUpdate): {
      ...
      fprintf(stdout,
              "[UDP_Receiver] Applied VehicleUpdatePacket of %d bytes "
              "from id %d... \n",
              ph->size, vup->id);
    END:
      pthread_mutex_unlock(&mutex);
...
void *UDPSender(void *args) {
  ...
  while (connectivity && exchange_update) {
    ...
    while (client != NULL) {
      ...
      while (tmp != NULL) {
        if (tmp->is_addr_ready && (abs(tmp->x - client->x) <= HIDE_RANGE &&
...
int main(int argc, char **argv) {
  ...
  debug_print("[Main] TCP loop ended... \n");
