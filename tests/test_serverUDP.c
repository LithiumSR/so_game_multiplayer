...
int main(int argc, char **argv) {
 ...
 if (argc != 2) {
    ...
    ret = bind(server_desc, (struct sockaddr*) &udp_server, sizeof(udp_server)); // binding dell'indirizzo
    ...
    while (1) {
        ...
        bytes_received = recvfrom(server_desc,buf_recv,BUFFSIZE,0, (struct sockaddr*)&client_addr,&addrlen);
