...
int TCP_Handler(int socket_desc, char* buf_rcv, Image* texture_map,
                Image* elevation_map) {
...
void* session(void* args) {
  ...
  while (1) {
    ...
    int ret =
        TCP_Handler(sock_fd, buf_rcv, arg->texture_map, arg->elevation_map);
...
int main(int argc, char** argv) {
  ...
  ERROR_HELPER(ret,
               "[MAIN THREAD] Impossibile settare l'opzione SO_REUSEADDR per "
               "socket server_desc");

  int sockaddr_len = sizeof(struct sockaddr_in);
  ret = bind(server_desc, (struct sockaddr*)&server_addr,
  ...
  ERROR_HELPER(ret,
               "[MAIN THREAD] Impossibile eseguire listen() su server_desc");
  ...
  while (1) {
    ...
    int client_desc = accept(server_desc, (struct sockaddr*)&client_addr,
                             (socklen_t*)&sockaddr_len);
    if (client_desc == -1 && errno == EINTR) continue;
    ERROR_HELPER(client_desc,
                 "[MAIN THREAD] Impossibile eseguire accept() su server_desc");
